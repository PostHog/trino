...
public class QueuedStatementResource
{
    ...
    {
        return postStatement(trino(statement), servletRequest, httpHeaders, externalUriInfo);
    }
            ...
            ExternalUriInfo externalUriInfo)
    {
        ...
        if (isNullOrEmpty(submission.originalText())) {
            throw new BadRequestException(submission.language() == TRINO ? "SQL statement is empty" : "HogQL query is empty");
        }

        Query query = registerQuery(submission, servletRequest, httpHeaders);
    ...
    private Query registerQuery(QuerySubmission submission, HttpServletRequest servletRequest, HttpHeaders httpHeaders)
    {
        ...
        Query query = new Query(submission, sessionContext, dispatchManager, queryInfoUrlFactory, tracer);
    ...
    {
        private final QuerySubmission submission;
        ...
        public Query(QuerySubmission submission, SessionContext sessionContext, DispatchManager dispatchManager, QueryInfoUrlFactory queryInfoUrlFactory, Tracer tracer)
        {
            this.submission = requireNonNull(submission, "submission is null");
        ...
        private void submitIfNeeded()
        {
            if (submissionGate.compareAndSet(NOT_SUBMITTED, SUBMITTED)) {
                ...
                creationFuture.setFuture(dispatchManager.createQuery(queryId, querySpan, slug, sessionContext, submission));
            }
        }
