...
public record HogQlExchangeRateSnapshot(
        int protocolVersion,
        int schemaVersion,
        long generation,
        String baseCurrency,
        ...
        List<ExchangeRate> rates)
{
    ...
    public HogQlExchangeRateSnapshot
    {
        if (protocolVersion != PROTOCOL_VERSION) {
            throw new IllegalArgumentException("unsupported HogQL exchange-rate protocol");
        }
        if (schemaVersion != SCHEMA_VERSION) {
            throw new IllegalArgumentException("unsupported HogQL exchange-rate schema");
        }
        if (generation <= 0) {
            throw new IllegalArgumentException("HogQL exchange-rate generation must be positive");
        }
        baseCurrency = requireNonNull(baseCurrency, "baseCurrency is null");
        if (!baseCurrency.equals(BASE_CURRENCY)) {
            throw new IllegalArgumentException("unsupported HogQL exchange-rate base currency");
        }
        if (decimalScale != DECIMAL_SCALE) {
            throw new IllegalArgumentException("unsupported HogQL exchange-rate decimal scale");
        }
        rates = List.copyOf(requireNonNull(rates, "rates is null"));
        if (rates.isEmpty()) {
            throw new IllegalArgumentException("HogQL exchange-rate snapshot has no rates");
        }
        if (rates.size() > MAXIMUM_RATES) {
            throw new IllegalArgumentException("HogQL exchange-rate snapshot exceeds rate limit");
        }
        ...
        for (ExchangeRate rate : rates) {
            ...
            if (rate.currency().equals(BASE_CURRENCY)) {
                ...
                if (!rate.unscaledRate().equals(UNSCALED_ONE)) {
                    throw new IllegalArgumentException("HogQL base-currency rate must equal one");
                }
            }
            if (previous != null && compare(previous, rate) >= 0) {
                throw new IllegalArgumentException("HogQL exchange rates are not strictly sorted");
            }
            previous = rate;
        }
        if (!baseCurrencyPresent) {
            throw new IllegalArgumentException("HogQL exchange-rate snapshot has no base-currency rate");
        }
    }
    ...
    public record ExchangeRate(String currency, String effectiveDate, String unscaledRate)
    {
        public ExchangeRate
        {
            ...
            if (!CURRENCY_PATTERN.matcher(currency).matches()) {
                throw new IllegalArgumentException("invalid HogQL exchange-rate currency");
            }
            try {
                if (!LocalDate.parse(effectiveDate).toString().equals(effectiveDate)) {
                    throw new IllegalArgumentException("noncanonical HogQL exchange-rate date");
                }
            }
            catch (DateTimeParseException e) {
                throw new IllegalArgumentException("invalid HogQL exchange-rate date", e);
            }
            if (!UNSCALED_RATE_PATTERN.matcher(unscaledRate).matches()) {
                throw new IllegalArgumentException("invalid HogQL exchange-rate value");
            }
        }
    }
}
