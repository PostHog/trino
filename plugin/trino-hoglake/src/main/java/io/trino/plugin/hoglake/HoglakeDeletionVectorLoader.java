 ...
 * <li>every deleted position must fall inside the Parquet file, and</li>
...
{
    /**
     * The largest deletion vector this connector pages into memory. A
     * deletion vector is a roaring bitmap plus a small footer, so this is
     * gigabytes of headroom over any real vector while keeping a hostile or
     * mistaken registration from driving an unbounded allocation.
     */
    ...
    /**
     * @param rowCount number of physical rows in the data file, from its
     *         Parquet footer; every deleted position must be inside it
     */
    static HoglakeDeletionVector load(TrinoFileSystem fileSystem, HoglakeSplit split, long rowCount)
    {
        ...
        byte[] bytes = read(fileSystem, path);
        HoglakeDeletionVector deletionVector = HoglakeDeletionVector.read(bytes, path);
        validate(deletionVector, split, rowCount);
        return deletionVector;
    }

    private static byte[] read(TrinoFileSystem fileSystem, String path)
    {
        TrinoInputFile inputFile = fileSystem.newInputFile(Location.of(path));
        try {
            long length = inputFile.length();
            if (length > MAX_DELETION_VECTOR_BYTES) {
                throw new TrinoException(
                        HOGLAKE_DELETION_VECTOR_NOT_FOUND,
                        "hoglake deletion vector %s is %d bytes, beyond the %d-byte limit this connector reads"
                                .formatted(path, length, MAX_DELETION_VECTOR_BYTES));
            }
            try (TrinoInput input = inputFile.newInput()) {
                byte[] bytes = new byte[toIntExact(length)];
    ...
    private static void validate(HoglakeDeletionVector deletionVector, HoglakeSplit split, long rowCount)
    {
        ...
        // Positions are file row ordinals: the vector's largest deleted
        // position must fall inside the file's real row count, so no
        // out-of-range position can silently delete nothing (or, under a
        // wrapping conversion, the wrong row).
